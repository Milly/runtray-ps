...
$testSut = (Split-Path -Leaf $MyInvocation.MyCommand.Path) -replace '\.Tests\.', '.'
. "$testRoot\$testSut"
...
Describe 'Start-Main' {
    ...
    It "should call Start-FromShortcut if `$script:Command is 'start'" {
        ...
        Assert-MockCalled Start-FromShortcut -Scope It -Times 1 -Exactly `
            -ParameterFilter { -Not $PassThru }
    }
    ...
    It "should call Install-Shortcut if `$script:Command is 'install'" {
        ...
        Assert-MockCalled Install-Shortcut -Scope It -Times 1 -Exactly `
            -ParameterFilter { -Not $PassThru }
    }
    ...
    It "should call Uninstall-Shortcut if `$script:Command is 'uninstall'" {
        ...
        Assert-MockCalled Uninstall-Shortcut -Scope It -Times 1 -Exactly
    }
    ...
    It "should call Start-GUI if `$script:Command is 'run'" {
        ...
        Assert-MockCalled Start-GUI -Scope It -Times 1 -Exactly
    }

    It 'should call Get-Help if $script:Command is default' {
        Start-Main

        Assert-MockCalled Get-Help -Scope It -Times 1 -Exactly `
            -ParameterFilter { $Name -eq $script:scriptPath }
    }
}

Describe 'Get-Config' {
    function New-TestConfig($obj) {
        ConvertTo-Json $obj > $testJsonFile
    }
    ...
    Context 'configuration file with the same name as the script exists' {
        BeforeEach {
            New-TestConfig (Merge-Config $validMinimumConfig @{
                name='my-foo-app'
            })
    ...
    Context 'required property exist' {
        BeforeEach {
            New-TestConfig $validMinimumConfig
        }
    ...
    Context 'wrong type property exist' {
        ...
        It 'should throws an exception with JSONPath' -TestCases $testCases {
            ...
            New-TestConfig (Merge-Config $validMinimumConfig $Config)
    ...
    Context 'optional property does not exist' {
        ...
        It 'should insert a default value' -TestCases $testCases {
            ...
            if ($Expected -is [array]) {
                Compare-Object $actual.$Prop $Expected | Should BeNullOrEmpty
            } else {
...
Describe 'Start-FromShortcut' {
    ...
    It 'should calls Start-Process' {
        ...
        Assert-MockCalled Start-Process -Scope It -Times 1 -Exactly `
            -ParameterFilter { $FilePath -eq $testShortcutPath -And -Not $PassThru }
    ...
    Context 'with -PassThru' {
        It 'returns process' {
            ...
            Assert-MockCalled Start-Process -Scope It -Times 1 -Exactly `
                -ParameterFilter { $FilePath -eq $testShortcutPath -And $PassThru }
...
Describe 'Install-Shortcut' {
    ...
    It 'should create a shortcut file' {
        ...
        Test-Path $testShortcutPath | Should Be $true
    }
    ...
    Context 'the shortcut file already exist' {
        ...
        It 'should create a shortcut file by overwritng' {
            ...
            Test-Path $testShortcutPath | Should Be $true -ErrorAction Stop
...
Describe 'Uninstall-Shortcut' {
    ...
    Context 'the shortcut file exist' {
        ...
        It 'removes the shortcut file' {
            ...
            Test-Path $testShortcutPath | Should Be $false
        }
    }
...
Describe 'Invoke-InMutex' -Tags slow {
    ...
    Context 'the mutex does not exist' {
        ...
        It 'should not invokes else-block' {
            ...
            $actual | Should BeExactly 'bar'
            $vars.elseBlockInvoked | Should Be $false
        }
    }
    ...
    Context 'the mutex already exist' {
        ...
        It 'should not invokes block' {
            ...
            $actual | Should BeNullOrEmpty
            $vars.blockInvoked | Should Be $false
        }
